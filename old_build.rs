...
fn main() {
    ...
    clang_cmd
        .arg("-O3")                  // maximum per-file optimization
        .arg("-flto")                // full link-time optimization
        .arg("-finline-functions")   // force inlining of functions
        .arg("-funroll-loops")       // unroll loops where profitable
        .arg("-ftree-vectorize")     // enable vectorization
        .arg("-fslp-vectorize")      // enable SLP vectorization
        .arg("-fPIC")                // keep position-independent code
        .arg("-c")
        .arg("-o")
        .arg(&obj_path)
        .arg(&static_fns_path)
